...
const DashboardView: React.FC<DashboardViewProps> = ({ profile, publishedVideos }) => {
  ...
  const triggerError = () => {
    throw new Error("Simulated system failure: The rendering engine encountered an unexpected null reference while processing the dashboard statistics.");
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-zinc-500">
        <h2 
          ...
          className="text-3xl font-bold mb-4 text-white cursor-pointer select-none"
        >
          Welcome to TubeAI
        </h2>
        {clickCount >= 5 && (
          <button 
            ...
            className="mb-4 px-4 py-2 bg-red-500/10 border border-red-500/20 text-red-500 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse"
          >
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h3 onClick={handleTitleClick} className="text-xl font-bold cursor-pointer select-none">Channel Overview</h3>
        {clickCount >= 5 && (
          <button 
            onClick={triggerError}
            className="px-4 py-2 bg-red-500/10 border border-red-500/20 text-red-500 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse"
          >
            Simulate System Failure
          </button>
        )}
